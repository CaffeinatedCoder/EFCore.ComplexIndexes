...
- **Fixed:** an `Npgsql:IndexSortOrder`/`IndexNullSortOrder` annotation on an index that also carries per-part sort options is now rejected rather than silently ignored by the package's SQL generator.
